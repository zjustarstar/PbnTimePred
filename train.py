...
from dataset import TimePredictionDataSet,TimePredictionDataSetTrain,TimePredictionDataSetTest
...
parser.add_argument('--dataset_train',
                    default='/home/cgim/wushukai/code/LeXin/LexinTimePrediction/Lexin-TimePrediction/file/train.json', type=str)
parser.add_argument('--dataset_test',
                    default='/home/cgim/wushukai/code/LeXin/LexinTimePrediction/Lexin-TimePrediction/file/test.json', type=str)
...
trainset = TimePredictionDataSetTrain(args.dataset_train)
trainloader = DataLoader(trainset, batch_size=args.train_batch, shuffle=True, num_workers=2, drop_last=True)
testset = TimePredictionDataSetTest(args.dataset_test)
testloader = DataLoader(testset, batch_size=args.test_batch, shuffle=True, num_workers=2, drop_last=True)
...
def train_model(model, model_type, criterion, optimizer, exp_lr_scheduler, early_stopping, save_interval, test_interval, checkpoints_dir, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for img, sehao, sekuai, area, pos, sekuai_dis, labels in dataloaders['train']:
            img = img.cuda() # 占位，不用管
            # 色号，色块，面积分布特征，位置分布特征，色号对应的色块数量分布特征
            sehao = sehao.cuda().unsqueeze(-1)
            sekuai = sekuai.cuda().unsqueeze(-1)
            area = area.cuda().unsqueeze(-1).float()
            pos = pos.cuda().unsqueeze(-1).float()
            sekuai_dis = sekuai_dis.cuda().unsqueeze(-1).float()


            labels = labels.cuda().unsqueeze(-1)
            results = model([sehao, sekuai, area, pos, sekuai_dis], model_type=model_type)
            ...
            train_loss += loss.data * sehao.size(0)
            train_img_num += sehao.size(0)
        ...
        if (epoch+1) % test_interval == 0:
            ...
            true_num_120 = 0
            true_num_180 = 0
            for img, sehao, sekuai, area, pos, sekuai_dis, labels in dataloaders['val']:
                img = img.cuda()
                sehao = sehao.cuda().unsqueeze(-1)
                sekuai = sekuai.cuda().unsqueeze(-1)
                area = area.cuda().unsqueeze(-1).float()
                ...
                with torch.no_grad():
                    ...
                    test_loss += nn.L1Loss()(results, labels).data * sehao.size(0)
                    test_img_num += sehao.size(0)
                    results = results.squeeze(-1).detach().cpu().numpy()
                    labels = labels.detach().squeeze().cpu().numpy()
